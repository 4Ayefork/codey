...
mod tests {
    ...
    #[test]
    fn parses_codey_provider_url() {
        let url = "codey://v1/import/provider?resource=provider&name=Example%20Provider&baseUrl=https%3A%2F%2Fexample-provider.com%2Fv1&apiKey=sk-test&wireApi=responses&relayMode=pureApi&configContents=bW9kZWxfcHJvdmlkZXIgPSAiQ29kZXlSdW50aW1lIgo=&authContents=eyJPUEVOQUlfQVBJX0tFWSI6InNrLXRlc3QifQo%3D";

        let request = request_from_url(url).unwrap();

        assert_eq!(request.name, "Example Provider");
        assert_eq!(request.base_url, "https://example-provider.com/v1");
    ...
    fn imports_provider_once_and_selects_it() {
        ...
        let request = ProviderImportRequest {
            ...
            base_url: "https://example-provider.com/v1/".to_string(),
    ...
    fn pending_provider_import_round_trips_and_clears() {
        ...
        assert_eq!(pending.name, "Example Provider");
        assert_eq!(pending.base_url, "https://example-provider.com/v1");
