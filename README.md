...
Codey 已将实际使用的 `Codey v0.4.3` core/data crate 固定在 `vendor/Codey`，生命周期和会话扫描优化也已直接合并其中。本地与 CI 构建不再需要同级外部源码目录、运行时补丁或从 GitHub 下载该依赖。上游提交、Codey 修改范围和许可证记录在 `vendor/Codey/UPSTREAM.md`。
